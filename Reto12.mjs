...
const getMinJump = (obstacles) => {
    ...
    let arrayPuntos = ['.','.','.','.','.','.','.','.','.','.','.']
    obstacles.forEach (arr => arrayPuntos[arr] = 'X')
    let longitudSalto = 1
    for (longitudSalto = 1; longitudSalto <= 10; longitudSalto++) {
        let longitudHallada = true, posicionSalto = 0
        while ((posicionSalto <= 10) && longitudHallada) {
            if (arrayPuntos[posicionSalto + longitudSalto] === 'X') {
                longitudHallada = false
            } else posicionSalto = posicionSalto + longitudSalto
        }
        if (longitudHallada) {
            break
        }
