...
const shouldBuyFidelity = (times) => {
    ...
    const precioTicket = 12, descuento = 0.75, precioTarjetaFidelidad = 250
    let precioDescuentoAcumulado = 0, precioConDescuento = 0
    const valorNormalEntradas = precioTicket * times

    for (let x = 1; x <= times; x++) {
        precioConDescuento = precioTicket * Math.pow(descuento, x)
        precioDescuentoAcumulado = precioDescuentoAcumulado + precioConDescuento
    }

   return valorNormalEntradas > precioTarjetaFidelidad + precioDescuentoAcumulado
}

console.log (shouldBuyFidelity(25))