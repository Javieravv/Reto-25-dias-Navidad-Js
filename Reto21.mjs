...
const canCarry = (capacity, trip) => {
    ...
    let giftCollect = 0
    let deliveryPoints = [], giftDeliver = []
    trip.forEach (routeData => {
      giftCollect = giftCollect + routeData[0]
      deliveryPoints.push(routeData[2])
      giftDeliver.push(routeData[0])
      let deliveryPoint = deliveryPoints.indexOf(routeData[1])
      if (deliveryPoint > -1) {
        giftCollect = giftCollect - giftDeliver[deliveryPoint]
      }
    })
    return giftCollect <= capacity
}
